...
impl AppState {
    ...
    pub async fn set_balance(&mut self, wallet_endpoint: String, user_pk: String) {
...

        {
            ...
            let _ = wallet_sdk::update_wallet(wallet_endpoint.clone(), &user_pk).await;
        }

        let balance = match wallet_sdk::get_balance_from_wallet(wallet_endpoint, &user_pk).await {
            Ok(resp) => {
