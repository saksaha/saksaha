...
async fn get_disc_args(idx: usize) -> DiscoveryArgs {
    let disc_args_1 = || async {
    ...
    let args = vec![disc_args_1];

    let a = args.get(idx).unwrap();
    let b = a().await;
...
pub(super) async fn create_disc(disc_idx: u16) -> (Arc<Discovery>, String) {
    ...
    let disc_args = get_disc_args(idx).await;
