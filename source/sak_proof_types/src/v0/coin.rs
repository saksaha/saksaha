...
impl OldCoin {
    ...
    pub fn new_dummy() -> Result<OldCoin, ProofTypeError> {
        let hasher = MiMC::new();
    ...
    pub fn compute_sn(&self) -> Result<Scalar, ProofTypeError> {
        let sn = {
...
impl NewCoin {
    ...
    pub fn compute_cm(&self) -> Result<Scalar, ProofTypeError> {
        {
            if self.r.is_none()
                || self.s.is_none()
                || self.v.is_none()
