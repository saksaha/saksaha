...
mod test {
    ...
    fn get_dummy_p2p_identity(
        public_key: PublicKey,
        public_key_str: String,
        secret: String,
    ) -> Arc<P2PIdentity> {
        let id = P2PIdentity::new(secret, public_key_str)
            .expect("Make a dummy p2p identity failed");

        Arc::new(id)
    }

    async fn get_dummy_peer_table() -> Arc<PeerTable> {
        let ps = PeerTable::init(Some(5))
            .await
            .expect("Make a dummy peer table failed");

        Arc::new(ps)
    }

    async fn dummy_initiate_handshake(
        public_key: PublicKey,
        public_key_str: String,
        src_sig: Signature,
        secret: String,
        ...
        let endpoint = format!("127.0.0.1:{}", p2p_port);
        let conn = match TcpStream::connect(&endpoint).await {
            Ok(s) => {
                let (c, _) = Connection::new(s).expect("Connection failed");
                c
            }
            Err(err) => {
                println!("conn failed: err: {}", err);
                return;
            }
        };
        ...
        let handshake_init_args = HandshakeInitArgs {
            addr_guard,
            p2p_port: disc_port,
            p2p_identity: get_dummy_p2p_identity(
                public_key,
                public_key_str,
                secret,
            ),
            p2p_peer_table: get_dummy_peer_table().await,
        };

        println!("successfully reachs in progress of handshake");

        super::initiate_handshake(handshake_init_args, conn)
            .await
            .unwrap();
    }
    ...
    async fn wrong_signature() {
        dbg!("signature has zero values");
        let public_key_str = "049c2e0051debfb5eb52b37c0444a27af07ca3642345097fe3\
        4f56409e41913f8e0fd8f518482fb3fade858ef54d8a1bc27b169e1d80bc2211243dfaafb1325ad5".to_string();

        let secret =
            "18a1f443bef8547077bf71a29d4690c87c1f599253260f0efc3b6ff58498b954"
                .to_string();

        let public_key =
            crypto::convert_public_key_str_into_public_key(&public_key_str)
                .unwrap();

        let dummy_buf = vec![0; 74];
        let src_sig = {
            let mut sig_bytes = [0u8; 71];
            sig_bytes.copy_from_slice(&dummy_buf[2..73]);

            Signature::from_der(&sig_bytes).unwrap()
        };

        dummy_initiate_handshake(public_key, public_key_str, src_sig, secret)
            .await;
    }

    #[tokio::test(flavor = "multi_thread")]
    #[should_panic]
    async fn wrong_public_key() {
        let public_key_str = "dummy of public key".to_string();
        let secret = "dummy of secret key".to_string();
        let public_key =
            crypto::convert_public_key_str_into_public_key(&public_key_str)
                .unwrap();
