...
impl SakMachine {
    pub async fn insert_into_pool(&self, tx_candidates: Vec<TxCandidate>) {
        for tx in tx_candidates.into_iter() {
            ...
            if let Err(err) = self.ledger.sync_pool.insert_tx(tx).await {
    ...
    pub(crate) async fn make_block_candidate(
        ...
        let bc = self
            .ledger
            .consensus
            .do_consensus(&self.ledger, tx_candidates)
            .await?;

        self.ledger.sync_pool.remove_tcs(&bc.tx_candidates).await?;
